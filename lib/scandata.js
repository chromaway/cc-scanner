...
var SQL = {
  create: {
    ...
    indices: {
      pk: 'DO $$ ' +
          ...
          '  ) THEN ' +
          '  CREATE INDEX scan_data_idx_pk ON scan_data (blockhash, txid); ' +
          'END IF; ' +
          'END$$;',
      height: 'DO $$ ' +
              'BEGIN ' +
  ...
  select: {
    lastHeight: 'SELECT height, blockhash FROM scan_data ' +
                ...
                '  LIMIT 1',
    blockTransactionIds: 'SELECT blockhash, txid FROM scan_data ' +
                         '  WHERE height = $1'
  },
  remove: {
    row: 'DELETE FROM scan_data WHERE blockhash = $1 AND txid = $2'
  }
}
    ...
    .then(function (latest) {
      ...
      return new Promise(function (resolve, reject) {
        function undo () {
          ...
          return self._storage.transaction(function () {
            return self._storage.executeSQL(SQL.select.blockTransactionIds, [currHeight])
              .then(function (rows) {
                if (rows.length === 0) {
                  return [null, null]
                }

                return [rows[0].blockhash, _.pluck(rows, 'txid')]
              })
          })
          .spread(function (blockhash, txids) {
            ...
            .then(function () {
              return self._storage.transaction(function () {
                return Promise.map(txids, function (txid) {
                  return self._storage.executeSQL(SQL.remove.row, [blockhash, txid])
                })
              })
            })
          })
